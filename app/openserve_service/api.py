...
api_root_url = "https://api.openserv.ai"
# api_key = os.getenv("OPENSERV_API_KEY")
print(f"API_BASE_URL: {api_root_url}")
# print(f"OPENSERV_API_KEY: {api_key}")
...
class APIClient: 
    ...
    async def ensure_session(self):
        if self.session is None:
            ...
            self.session = aiohttp.ClientSession(
                headers={'x-openserv-key': self.api_key},  # Set only the API key header by default
                connector=connector
            )
        return self.session
    ...
    async def upload_file(self, workspace_id: str, file_content: Union[str, bytes], 
        ...
        print(f"Uploading file {filename} to workspace {workspace_id} to path /workspaces/{workspace_id}/file")
